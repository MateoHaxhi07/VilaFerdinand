...
const Home = () => {
  ...
  const fetchTotalQuantity = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/sales/total-quantity?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}&sellers=${selectedSellers.map(s => s.value).join(",")}&sellerCategories=${selectedSellerCategories.map(cat => cat.value).join(",")}&articleNames=${selectedArticleNames.map(a => a.value).join(",")}&categories=${selectedCategories.map(cat => cat.value).join(",")}`
      );
      const data = await response.json();
  ...
  const fetchMostSoldItems = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/sales/most-sold-items?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}&sellers=${selectedSellers.map(s => s.value).join(",")}&sellerCategories=${selectedSellerCategories.map(cat => cat.value).join(",")}&articleNames=${selectedArticleNames.map(a => a.value).join(",")}&categories=${selectedCategories.map(cat => cat.value).join(",")}`
      );
      const data = await response.json();
  ...
  const fetchMostSoldItemsByPrice = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/sales/most-sold-items-by-price?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}&sellers=${selectedSellers.map(s => s.value).join(",")}&sellerCategories=${selectedSellerCategories.map(cat => cat.value).join(",")}&articleNames=${selectedArticleNames.map(a => a.value).join(",")}&categories=${selectedCategories.map(cat => cat.value).join(",")}`
      );
      const data = await response.json();
  ...
  const fetchDailySales = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/sales/daily-sales?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}&sellers=${selectedSellers.map(s => s.value).join(",")}&sellerCategories=${selectedSellerCategories.map(cat => cat.value).join(",")}&articleNames=${selectedArticleNames.map(a => a.value).join(",")}&categories=${selectedCategories.map(cat => cat.value).join(",")}`
      );
      const data = await response.json();
      setDailySales(data);
    } catch (error) {
  ...
  const fetchCategories = async () => {
    try {
      let url = "http://localhost:5000/sales/categories?";
      const queryParams = [];
  ...
  const fetchArticleNames = async () => {
    try {
      let url = "http://localhost:5000/sales/article-names?";
      const queryParams = [];
  ...
  <div>
    ...
    <div style={{ height: 400, marginTop: "2rem" }}>
      ...
      <ResponsiveBar
        ...
        colors={() => "#00008B"} // Custom color function for dark blue
          ...
          axis: {
            ticks: {
              text: {
                fontSize: 12,
                fontWeight: "bold",
                fill: "#000000", // Dark black color for axis labels
            ...
            legend: {
              text: {
                fontSize: 14,
                fontWeight: "bold",
                fill: "#000000", // Dark black color for axis legends
                fontFamily: "Arial, sans-serif", // Professional font
              },
            },
          },
