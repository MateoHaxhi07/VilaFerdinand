...
export function ComparisonChart({
  ...
  useEffect(() => {
    ...
    fetchComparison();
  }, [startDate, endDate, apiUrl, selectedSellers, selectedSellerCategories, selectedArticleNames, selectedCategories, selectedHours]);
  ...
  const startStr = moment(startDate).format("YYYY-MM-DD");
  const endStr = moment(endDate).format("YYYY-MM-DD");
  const allDatesThisYear = (() => {
    const dates = [];
    let curr = moment(startStr);
    const last = moment(endStr);
  ...
  const chartOption = {
    ...
    yAxis: { type: 'value' },
    series: [
      { name: 'This Year', type: 'bar', barGap: '30%', data: seriesThisYear },
      { name: 'Last Year', type: 'bar', data: seriesLastYear }
    ]
  };
...
//#endregion
//#region ShitjetAnalitikeCardTop7
...
//#endregion
//#region ShitjetAnalitikeFullPage
---------------------------------------------------------- */
